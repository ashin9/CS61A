...
def balanced(m):
    ...
    # if is_mobile(left_arm):
    #     left_weight = total_weight(left_arm)
    #     balanced(left_arm)
    # if is_planet(left_arm):
    #     left_weight = size(left_arm)

    # if is_mobile(right_arm):
    #     right_weight = total_weight(right_arm)
    #     balanced(right_weight)
    # if is_planet(right_arm):
    #     right_weight = size(right_arm) 
    ...
    torque_left, torque_right = length(left(m)) * total_weight(end_left), length(right(m)) * total_weight(end_right)
    # 同时满足 2 个条件
...
def has_path(t, word):
    ...
    if word[0] == label(t):
        ...
        for b in branches(t):
            # 区别于直接 return has_path(b, word[1:]), 可能提前 return false
