...
def free_bacon(score):
    ...
    #### 自己思路: 用 while
    # while not (pi > pow(10, score) and pi < pow(10, score + 1)):
    #     pi = pi // 10

    #### 官方思路: 根据提示, pi 整除 10 的 score 次方
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            if not extra_turn(score0, score1):
                who=other(who)
        # if who ==1 和 else 执行逻辑不同, if-else 只会选择一个, 而俩 if 则会顺序判断会导致逻辑错误
        else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        ...
        av_score = averaged_roll_dice(i, dice)
