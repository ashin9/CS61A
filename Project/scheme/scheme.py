...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # PROBLEM 20, 识别是否为 Macro, 并展开宏, 再递归评估结果
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # 为什么不是 expressions.rest ? 它是个 Pair . 以 do_define_form(read_line("(x (+ 2 8))"), env) 为例: 
        # 通过 read_line 会被解析成 Pair 对象 : Pair('define', Pair('x', Pair(Pair('+', Pair(2, Pair(8, nil))), nil)))
        # expressions.first               -> 'x'
        # expressions.rest                -> Pair(Pair('+', Pair(2, Pair(8, nil))), nil)
        # expressions.rest.first          -> Pair('+', Pair(2, Pair(8, nil)))
        ...
        # Pair(formals, body), 没想到
        env.define(fn_name, do_lambda_form(Pair(formals, body), env))
        return fn_name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    # (quote (+ x 2))
    # expressions = Pair(Pair('+', Pair('x', Pair(2, nil))), nil)
    # 只需要发你第一个即可
...
def do_or_form(expressions, env):
    ...
    #     return val
    # # if this is the last expression, evaluate with tail=True!
    # if expressions.rest is nil:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        values = Pair(scheme_eval(bindings.first.rest.first, env), values) # 求值, rest.first 可能是表达式比如 (y (* x 10))
